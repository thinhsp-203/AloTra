...
public class CartServiceImpl implements CartService {
    ...
    public CartItem addToCart(List<CartItem> cart, int productId, int quantity, 
        ...
        if (!"Mặc định".equals(finalSizeName)) {
            ProductSize productSize = productSizeDao.findByProductIdAndSizeName(productId, finalSizeName);
            if (productSize != null) {
        ...
        if (toppingParam != null && !toppingParam.isBlank()) {
            ...
            if (!toppingQuantities.isEmpty()) {
                List<Topping> selectedToppings = toppingDao.findByIds(new ArrayList<>(toppingQuantities.keySet()));
    ...
    public boolean updateItemDetails(List<CartItem> cart, int oldProductId, 
        ...
        Product p = productDao.findById(oldProductId);
        if (p == null) {
            return false;
        }
        ...
        if (!"Mặc định".equals(finalNewSize)) {
            ProductSize ps = productSizeDao.findByProductIdAndSizeName(oldProductId, finalNewSize);
            if (ps != null) {
