...
public class CartController extends HttpServlet {
  ...
  private void add(HttpServletRequest req, HttpServletResponse resp) throws IOException {
    ...
    try {
      ...
      int qty = 1;
      String[] toppingIds = req.getParameterValues("topping");
      ...
      try {
        ...
        if (toppingIds != null && toppingIds.length > 0) {
            List<Integer> ids = Arrays.stream(toppingIds)
                                      .map(Integer::parseInt)
                                      .sorted() // Sắp xếp để đảm bảo "1,2" và "2,1" là như nhau
            ...
            toppingsCost = selectedToppings.stream()
                                           .map(Topping::getPrice)
                                           .reduce(BigDecimal.ZERO, BigDecimal::add);

            toppingsCsv = selectedToppings.stream()
                                          .map(Topping::getTopping_name)
        ...
        resp.setContentType("application/json; charset=UTF-8");
        String newItemJson = "{\"productId\":" + ci.getProductId() +
                             ",\"productName\":\"" + escapeJson(ci.getProductName()) + "\"" +
                             ",\"thumbnail\":\"" + escapeJson(ci.getThumbnail()) + "\"" +
